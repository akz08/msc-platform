...
bool camHelper::calcUndistort(Mat inputMatrix)
{
    ...
    cv::Size boardSize = cv::Size(5, 8); // size of board (width x height)
    ...
    while(calibrating)
    {
        ...
        if(foundChessboard)
        {
            ...
            cornerSubPix(greyCameraMat, corners, cv::Size(11,11), cv::Size(-1,-1), TermCriteria( CV_TERMCRIT_EPS+CV_TERMCRIT_ITER, 30, 0.1));
        ...
        putText(calibCameraMat, msg, cv::Point(5,15), 1, 1,Scalar(0,255,0));
...
bool camHelper::loadPerspective()
{
    ...
    destinationSize = cv::Size(dstWidth, dstHeight);
    return true;
}
...
void camHelper::onMouse(int event, int x, int y, int flags)
{
    switch (event) {
        case CV_EVENT_LBUTTONDOWN:
        {
            ...
            {
                circle(perspectiveMat, cv::Point(x,y), 10, Scalar(0,0,0), 3, 8);
...
bool camHelper::calcPerspective(Mat inputMatrix, string winName, int calcType)
{
    ...
    switch(calcType) // choose how source points to be determined
    {
        case(CHESSBOARD):
        {
            ...
            int board_w = 5;
            int board_h = 8;
            cv::Size boardSize = cv::Size(board_w,board_h);
        ...
        case(CLICKRECTANGLE):
        {
            ...
            while(!enoughPerspectivePoints)
            {
                ...
                putText(perspectiveMat, msg, cv::Point(5,15), 1, 1,Scalar(0,255,0));
            ...
            destinationSize = cv::Size(scaled_w, scaled_h);
