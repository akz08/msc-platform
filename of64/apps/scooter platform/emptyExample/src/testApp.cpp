    ...
    if (phidget.isConnected){
        ...
        if (bridgeValuesArray[currentBridge].calculated){
            ...
            if(logEnabled)
            {
                ...
                phidgetRawFile << bridgeValuesArray[0].currentCalibratedValue << ",";
                phidgetRawFile << bridgeValuesArray[1].currentCalibratedValue << ",";
                phidgetRawFile << bridgeValuesArray[2].currentCalibratedValue << ",";
                phidgetRawFile << bridgeValuesArray[3].currentCalibratedValue << ",";
                phidgetRawFile << clockElapsed << endl;
        ...
        if(logEnabled)
        {
            ...
            float clockElapsed = (clock() - logStartTime) / 10000.0f ;
            phidgetCaliFile << bridgeValuesArray[0].currentValue << ",";
            phidgetCaliFile << bridgeValuesArray[1].currentValue << ",";
            phidgetCaliFile << bridgeValuesArray[2].currentValue << ",";
            phidgetCaliFile << bridgeValuesArray[3].currentValue << ",";
...
void testApp::eventGUISetup(ofxUIEventArgs &e){
    ...
    if(name == "enable calibration saving"){
...
void testApp::eventGUIPlatform(ofxUIEventArgs &e){
    ...
    else if(name == "enable logging")
    {
        ofxUIToggle *toggle = (ofxUIToggle *) e.widget;
        if(toggle->getValue())
        {
            // prepare files for logging session
            string temp_filename = baseCsvFolder + logSessionName + "rawLoadCell.csv";
    ...
    else if(name == "session name")
    {
        ...
        if(textinput->getTriggerType() == OFX_UI_TEXTINPUT_ON_UNFOCUS)
        {
            logSessionName = textinput->getTextString();
        }
    }
}
...
void testApp::setGUIPlatform(){
    ...
    logSessionInput->setAutoClear(false);
    guiPlatform->addWidgetDown(logSessionInput);
    logTrigger = new ofxUIToggle(16, 16, false, "enable logging");
    guiPlatform->addWidgetDown(logTrigger);
    logTimeElapsed = new ofxUILabel("time elapsed", OFX_UI_FONT_SMALL);
    guiPlatform->addWidgetDown(logTimeElapsed);
...
void testApp::setGUISetup(){
    ...
    guiSetup->addWidgetDown(new ofxUIToggle(16, 16, false, "enable calibration saving"));
